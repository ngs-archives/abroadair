...
jQuery.fn.combobox = function(options)
{
	...
	return this.each(
		function()
		{
			...
			var _valueDisplayContainerEnforcedStyle = "position:relative;";
			...
			///		jQuery('#container')[0].outerWidth; // 154px (2px border on the left and right)
			///		setInnerWidth(jQuery('#container'), 120);
			///		jQuery('#container').width(); // 116px
			///		jQuery('#container')[0].outerWidth; // 120px (2px border on the left and right)
			///</summary>				
			function setInnerWidth(elementJQuery, width)
			{
				var differenceWidth = (elementJQuery[0].outerWidth - elementJQuery.width());
			...
			{
				var differenceheight = (elementJQuery[0].outerHeight - elementJQuery.height());
			...
			function recursivelyBuildList(parentJQuery, childrenOptionsJQuery)
			{
				childrenOptionsJQuery.each(
					function()
					{
						...
						var width = (offsetLeft + builtDropDownItemJQuery[0].outerWidth);
			...
			function updateDropDownListWidth()
			{
				...
				var dropdownListWidth = _containerJQuery[0].outerWidth;
			...
			{
				var displayValueHeight = _valueDisplayJQuery[0].outerHeight;
			...
			function applyLayout()
			{
				...
				var displayValueWidth = (_valueDisplayContainerJQuery.width() - _dropDownButtonJQuery[0].outerWidth);
				setInnerWidth(_valueDisplayJQuery, displayValueWidth);
				var dropDownButtonHeight = _dropDownButtonJQuery[0].outerHeight;
				...
				var leftPosition = (currentLeftPosition - (_containerJQuery[0].outerWidth - _containerJQuery.width()));
			...
			function getDropDownListTop()
			{
				...
				var dropdownListHeight = _dropDownListJQuery[0].outerHeight;
				var comboboxBottom = (comboboxTop + _containerJQuery[0].outerHeight);
					...
					if (_dropDownListJQuery.is(":hidden"))
					{
						...
						switch (settings.animationType)
						{
							case "slide":
								...
								{
									var comboboxTop = _containerJQuery.position().top;
									var containerHeight = _containerJQuery[0].outerHeight;
						...
						switch (settings.animationType)
						{
							case "slide":
								if (_downdownListPositionIsInverted)
								{
									...
									slideDown(comboboxTop - _containerJQuery[0].outerHeight);
								}
								else
								{
